...
class LMUCell(Layer):
    ...
    def __init__(self,
        ...
        self.state_size = (self.units, self.udim * self.order)
    ...
    def build(self, input_shape):
        ...
        self.input_encoders = self.add_weight(
            name='input_encoders',
            shape=(self.input_dim, self.udim),
        ...
        self.memory_encoders = self.add_weight(
            ...
            shape=(self.order * self.udim, self.udim),
        ...
        self.memory_kernel = self.add_weight(
            name='memory_kernel',
            shape=(self.order * self.udim, self.units),
    ...
    def call(self, inputs, states):
        ...
        M = K.reshape(m, (-1, self.udim, self.order))
        m = K.reshape(M + K.dot(M, self.AT) + 
            K.dot(K.reshape(u, (-1, self.udim, 1)), self.BT),
             K.shape(m))
